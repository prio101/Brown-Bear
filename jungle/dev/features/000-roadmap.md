...
| **012** — Prompt Palace at `/prompts`: what was asked, the answer, and what each prompt sits near — prior prompts scored against the cache cutoff, and the knowledge chunks a retrieval lookup would have injected. Adds `machine` to `/ext/exchange` | Attribution is a claim, not a fact: the edge authenticates one shared secret for every machine, so `machine` is self-declared until G2 lands per-client identity. Only prompts stored after the hook is redeployed carry one |
| **011** — preview zoom on `/files`: a magnifying lens over images, the PDF viewer's own zoom, and a gallery over image files. `?original=1` on the preview route | No hover lens over a PDF — a framed PDF's pixels and scroll position are unreadable from the page, so a lens there would magnify the wrong region silently. pdf.js would fix it for ~1MB and is recorded as an open question, not taken |
